...
func NewController(
	...
	appleInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, new interface{}) {
			logger.Info("Updating apple", "new", new)
			controller.enqueueApple(new)
		},
		DeleteFunc: controller.enqueueApple,
	})
	...
	deploymentInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, new interface{}) {
			logger.Info("Updating deployment", "new", new)
...
func (c *Controller) processNextWorkItem(ctx context.Context) bool {
	...
	logger.Info("Processing next work item", "obj", obj)
	if shutdown {
		return false
	}
...
func (c *Controller) syncHandler(ctx context.Context, key string) error {
	...
	// Get the deployment with the name specified in Apple CR
	deployment, err := c.deploymentsLister.Deployments(apple.Namespace).Get(fmt.Sprintf("%s-%s", apple.Name, apple.Spec.Type))
	if errors.IsNotFound(err) {
		// Create the deployment
		deployment = newDeployment(apple)
		_, err = c.kubeclientset.AppsV1().Deployments(apple.Namespace).Create(ctx, deployment, metav1.CreateOptions{})
	}
	if err != nil {
		return err
	}

	// Update the Apple status
	appleCopy := apple.DeepCopy()
	if deployment.Status.ReadyReplicas == deployment.Status.Replicas {
		appleCopy.Status.State = "Running"
	} else {
		appleCopy.Status.State = "Pending"
	}

	// Update Apple status
	_, err = c.sampleclientset.ExampleV1alpha1().Apples(apple.Namespace).UpdateStatus(ctx, appleCopy, metav1.UpdateOptions{})
	if err != nil {
		return err
	}

	logger.Info("Successfully synced apple", "name", apple.Name, "status", appleCopy.Status.State)
	return nil
}
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{
						{
							Name:  "apple",
							Image: "nginx:latest", // Assuming image tag based on type
						},
					},
				},
			},
